...
# ------------------------
# Game helpers (host uses full state; players render snapshot)
# ------------------------
...
def compute_remaining_for_group(group_name: str, r: int, req_for_round: float) -> float:
    ...
    for L in logs:
        for t, d in L["actions"]:
            if t == "cash": used += float(d.get("used", 0.0))
            elif t in ("repo","sell","redeem_td"): used += float(d.get("use", 0.0))
...
tickers = all_tickers[:3]  # show first 3
...
def _host_publish_snapshot():
    # Create a lightweight snapshot for all players to see (and for player input limits)
    _, px = base_prices_for_round(r, df, tickers)
    groups_out = []
    for p in st.session_state.portfolios:
        s = p.summary(px)
        positions = {t: float(p.pos_qty.get(t, 0.0)) for t in tickers}
        groups_out.append({
            "name": p.name,
            "summary": {
...
def _host_apply_single_action(p: Portfolio, req_all: float, px: Dict[str, float], action: Dict[str, Any]):
    """Apply a queued action to portfolio p and append to logs."""
    ...
    if repo_tick != "(none)" and repo_amt > 0 and rem_left > 0:
        price = float(px.get(repo_tick, 0.0))
        max_amt = p.pos_qty.get(repo_tick, 0.0) * price
        repo_amt = min(repo_amt, max_amt)
        if repo_amt > 0:
            info = _safe_repo_call(p, repo_tick, repo_amt, price, r, daily_rates_for_round(r)[0])
            got = float(info["got"])
            ...
            history.append(("repo", {
                ...
                "repo_id": info["repo_id"], "rate": daily_rates_for_round(r)[0]
            }))
    ...
    if invest_amt > 0:
        ...
        if invest_amt > 0:
            td_ids = _safe_invest_td(p, invest_amt, r, daily_rates_for_round(r)[1])
            history.append(("invest_td", {
                "amount": round(invest_amt, 2),
                "td_ids": td_ids,
                "rate": daily_rates_for_round(r)[1]
            }))
...
def _host_consume_actions_and_publish(req_all: float, px: Dict[str, float]):
    ...
    if actions:
        ...
        for a in actions:
            ...
            if rr != r:  # ignore actions for other rounds
                continue
            if 0 <= gi < len(st.session_state.portfolios):
                _host_apply_single_action(st.session_state.portfolios[gi], req_all, px, a.get("inputs", {}))
...
# ------------------------
def _render_endgame(final_px: Dict[str, float]):
...
if role == "Host":
    ...
    _, prices = base_prices_for_round(r, df, tickers)
    ...
    _host_consume_actions_and_publish(req_all, prices)
...
if role == "Player":
    ...
    tickers = snapshot.get("tickers", tickers)
...
# ------------------------
date_str, _prices_tmp = base_prices_for_round(r, df, tickers)
...
if role == "Host":
    ...
    # render from snapshot (read-only view)
    snapshot = _json_read(SNAPSHOT_PATH, {})
    ...
    for g, c in enumerate(cols):
        ...
        with c:
            ...
            rem = 0.0  # not tracked in snapshot per-group; keep the bar full if no withdrawal
...
# ------------------------
if role == "Host":
    # full-power tabs (host edits directly)
    _, prices = base_prices_for_round(r, df, tickers)
    req_all = float(st.session_state.withdrawals[r])
    ...
    for g, tab in enumerate(tabs):
        ...
        with tab:
            ...
            st.selectbox("Repo ticker", ["(none)"] + tickers, index=(["(none)"] + tickers).index(st.session_state.get(repo_tick_key, "(none)")), key=repo_tick_key)
            ...
            if st.session_state.get(repo_tick_key, "(none)") != "(none)" and st.session_state.get(repo_amt_key, 0.0) > 0:
                ...
                if prices.get(rt, 0.0) > 0:
                    st.caption(f"≈ {ra/prices[rt]:,.2f} units of {rt}")
            ...
            st.selectbox("Sell ticker", ["(none)"] + tickers, index=(["(none)"] + tickers).index(st.session_state.get(sell_tick_key, "(none)")), key=sell_tick_key)
            ...
            st.selectbox("Buy ticker", ["(none)"] + tickers, index=(["(none)"] + tickers).index(st.session_state.get(buy_tick_key, "(none)")), key=buy_tick_key)
    ...
    if st.session_state.pending_apply is not None or st.session_state.pending_clear is not None:
        ...
        if st.session_state.pending_apply is not None:
            ...
            # re-use host apply by emulating the same input capture as players would send
            ...
            _host_apply_single_action(p, req_all, prices, inputs)
            # reset inputs
...
if role == "Player":
    ...
    for gi, tab in enumerate(tabs):
        with tab:
            ...
            # (Players do not see per-group 'remaining' bar precisely—kept simple)
    ...
    if not you_own:
        ...
        # Inputs (limited by snapshot positions where relevant)
        ...
        def _submit_player_action():
            # Basic local constraint to avoid wild inputs (sell > position)
...
if st.session_state.role == "Host":
    with rgt:
        if st.button("Next Round ▶️"):
            # check cover-all (host has authoritative logs)
            _, prices = base_prices_for_round(r, df, tickers)
            req_all = float(st.session_state.withdrawals[r])
            all_covered = all(
