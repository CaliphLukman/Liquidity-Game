...
# Shared-state files (single game, no rooms)
SHARED_STATE_PATH = ".shared_state.json"  # tiny control file host writes, players read
UPLOADED_CSV_PATH = ".uploaded.csv"       # the host's CSV saved so all sessions can load it

# ------------------------
# Small helpers (safe rerun + shared json)
# ------------------------
def _safe_rerun():
    try:
        st.rerun()
...
def _write_shared(payload: Dict[str, Any]) -> None:
    tmp = SHARED_STATE_PATH + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(payload, f)
    os.replace(tmp, SHARED_STATE_PATH)

def _mutate_shared(fn) -> Dict[str, Any]:
    state = _read_shared()
    new_state = fn(state or {})
    if not isinstance(new_state, dict):
        new_state = state or {}
    _write_shared(new_state)
    return new_state
...
# safe adapters around game_core calls
...
if role == "Host":
    ...
    st.session_state.player_name = st.sidebar.text_input("Your name (to claim a group)", value=st.session_state.player_name or "")
    start_clicked = False
    end_clicked = False
    uploaded = None
    shared = _read_shared()
...
if role == "Host" and start_clicked:
    if uploaded is None:
        ...
        if "date" not in df.columns or len([c for c in df.columns if c != "date"]) < 3:
            ...
            # Save CSV so all sessions can load it
            ...
            # Random initial TD allocation (Round 1 only)
            ...
            # Update shared control so Players flip from "waiting" to game
            _write_shared({
                "initialized": True,
                "rng_seed": seed_val,
                "rounds": rounds_val,
...
if role == "Host" and end_clicked and st.session_state.initialized:
    # Push end-game to shared and local
    _mutate_shared(lambda s: {**s, "current_round": s.get("rounds", st.session_state.rounds), "ts": _now_ts()})
...
# Players: bootstrap from shared file as soon as host starts
if role == "Player" and not st.session_state.initialized:
    ...
    if not os.path.exists(UPLOADED_CSV_PATH):
        st.info("Waiting for Host CSV to be available…")
        st.stop()
    df = pd.read_csv(UPLOADED_CSV_PATH)

    # Initialize local session from shared
    _init_state()
    st.session_state.initialized = True
    ...
    # Build portfolios locally to match host’s #groups and initial makeup
    tickers_all = [c for c in df.columns if c != "date"]
    date0, prices0 = base_prices_for_round(0, df, tickers_all)
    base_portfolios = init_portfolios(tickers_all, prices0, total_reserve=200000.0)
    st.session_state.portfolios = base_portfolios[:st.session_state.num_groups]

    # Apply deterministic initial TD allocation using host seed transform
    random.seed(st.session_state.rng_seed ^ 0xA5A5)
    for p in st.session_state.portfolios:
        frac = random.uniform(0.10, 0.30)
        amt = round(max(0.0, p.current_account) * frac, 2)
        if amt > 0:
...
# If still not initialized (Host forgot to start)
...
# Sync current round from Host to Players on every refresh
if role == "Player":
    shared = _read_shared()
...
# ------------------------
# Group claiming (Players)
# ------------------------
if role == "Player" and st.session_state.initialized and st.session_state.portfolios:
    available_groups = [p.name for p in st.session_state.portfolios]
    current_choice = min(st.session_state.player_group_index, len(available_groups)-1)
    chosen = st.sidebar.selectbox("Select your Group", available_groups, index=current_choice)
    st.session_state.player_group_index = available_groups.index(chosen)
    st.sidebar.caption(f"You control: **{chosen}**")
...
for g, tab in enumerate(tabs):
    ...
    with tab:
        ...
        you = " (You)" if (role == "Player" and editable) else ""
        st.markdown(f"### {p.name}{you}")
...
# ------------------------
# Controls (Host drives rounds for everyone)
# ------------------------
st.divider()
    ...
    if role != "Host":
        ...
        if not all_covered:
            ...
            if st.session_state.current_round + 1 < st.session_state.rounds:
                ...
                st.session_state.current_round = st.session_state.rounds
            # push to shared so Players follow
            _mutate_shared(lambda s: {**s,
