...
SNAPSHOT_FILE = "snapshot.json"  # <-- players look for this; publish on Start/Reset
...
# ------------- tiny helper: publish a snapshot so players stop "waiting" -------------
def _publish_snapshot_flag():
    try:
        with open(SNAPSHOT_FILE, "w", encoding="utf-8") as f:
            json.dump({"published": True, "ts": time.time()}, f)
    except Exception:
        # If writing fails, we silently ignore to avoid breaking Host flow
        pass
...
if role == "Host":
    ...
    if st.session_state.initialized and st.session_state.portfolios:
        ...
        # Players look for snapshot.json existence downstream in your code;
        # we keep the same UX text.
...
if role == "Host" and start_clicked:
    if uploaded is None:
        ...
        if "date" not in df.columns or len([c for c in df.columns if c != "date"]) < 3:
            ...
            # 🔔 NEW: publish a simple snapshot flag so players stop "waiting"
            _publish_snapshot_flag()
